...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const [_, error] = await login(pseudo, password);
    if (error) {
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen w-screen bg-gray-900 grid place-items-center">
        <form
          onSubmit={handleSubmit}
          className="flex flex-col gap-3 px-10 w-full md:max-w-lg"
        >
